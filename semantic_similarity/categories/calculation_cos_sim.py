...
from database.utils.hotTechCompanies.schema_create import Company, company_category, Category, Similar
...
# Ja tentei: media, media das maiores sims, score maximo gerais
...
for i in comp_group_similar:
    ...
    # Separa apenas scores maiores que 0.4
    # XX a nao ser que a empresa tenha poucos (< 5) similares e nesse caso seleciona os 5 maiores entre 0.3 e 0.4
...
for i in comp_group_similar:
    for j in comp_group_similar[i]:
        for k in comp_group_similar[j[0]]:
             if k[0] == j[0]:
                comp_group_similar[j].remove(k)

with Session.begin() as session:
    for key in comp_group_similar:
        value = comp_group_similar[key]
        for c in value:
            session.add(Similar(company_1_uuid=key.uuid, company_2_uuid=c[0].uuid, score=c[1]))
